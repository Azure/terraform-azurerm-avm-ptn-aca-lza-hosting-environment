...
### [Windows VM with Custom Certificate](./windows-vm-custom-cert/)
Shows how to deploy with a Windows jumpbox VM and custom TLS certificates without using Application Gateway for ingress.

**Features**:
- Windows VM with password authentication
- Custom TLS certificate generation and management
...
**Use Case**: Windows-centric environments, custom certificate requirements, or scenarios where Application Gateway is not needed.
...
- Custom domain with TLS termination using Key Vault certificates
...
| Feature | Default | Hub-Spoke Linux | Windows Custom Cert | Bastion Zone Redundant | Minimal | Complex Network Appliance |
|---------|---------|----------------|--------------------|-----------------------|---------|-------------------------|
